...
func IsValid(m model.Money) bool {
	return signMatches(m) && validNanos(m.Nanos)
}

func signMatches(m model.Money) bool {
...
func Must(v model.Money, err error) model.Money {
	if err != nil {
		panic(err)
	}
	return v
...
// both).
func Sum(l, r model.Money) (model.Money, error) {
	if !IsValid(l) || !IsValid(r) {
		return model.Money{}, ErrInvalidValue
	} else if l.CurrencyCode != r.CurrencyCode {
		return model.Money{}, ErrMismatchingCurrency
	}
...
// to itself n-1 times.
func MultiplySlow(m model.Money, n uint32) model.Money {
	out := m
	for n > 1 {
		out = Must(Sum(out, m))
		n--
