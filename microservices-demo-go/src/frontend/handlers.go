...
import (
	...
	"github.com/GoogleCloudPlatform/microservices-demo-go/src/frontend/money"
	"github.com/GoogleCloudPlatform/microservices-demo-go/src/frontend/validator"
...
func (fe *frontendServer) viewCartHandler(w http.ResponseWriter, r *http.Request) {
	...
	totalPrice := model.Money{CurrencyCode: currentCurrency(r)}
...
func (fe *frontendServer) placeOrderHandler(w http.ResponseWriter, r *http.Request) {
	...
	order, err := fe.checkoutSvc.PlaceOrder(r.Context(), &model.PlaceOrderRequest{
		Email: payload.Email,
	...
	log.WithField("order", order.OrderId).Info("order placed")
...
func (fe *frontendServer) chooseAd(ctx context.Context, ctxKeys []string, log logrus.FieldLogger) *model.Ad {
	ads, err := fe.adSvc.GetAds(ctx, ctxKeys)
	if err != nil {
...
func cartIDs(c []*model.CartItem) []string {
...
func cartSize(c []*model.CartItem) int {
	cartSize := 0
	for _, item := range c {
...
func renderMoney(m model.Money) string {
...
// Helper methods that wrap client calls (formerly rpc.go logic, now inlined or helpers)
// rpc.go logic is mostly wrappers around client calls.
// I will implement these as helpers on frontendServer to keep the handler logic clean and close to original.

func (fe *frontendServer) getCurrencies(ctx context.Context) ([]string, error) {
	currs, err := fe.currencySvc.GetSupportedCurrencies(ctx)
	if err != nil {
		return nil, err
	}
	var out []string
...
func (fe *frontendServer) getProducts(ctx context.Context) ([]*model.Product, error) {
	return fe.productCatalogSvc.ListProducts(ctx)
}

func (fe *frontendServer) getProduct(ctx context.Context, id string) (*model.Product, error) {
	return fe.productCatalogSvc.GetProduct(ctx, id)
}

func (fe *frontendServer) getCart(ctx context.Context, userID string) ([]*model.CartItem, error) {
	return fe.cartSvc.GetCart(ctx, userID)
}

func (fe *frontendServer) emptyCart(ctx context.Context, userID string) error {
	return fe.cartSvc.EmptyCart(ctx, userID)
}

func (fe *frontendServer) insertCart(ctx context.Context, userID, productID string, quantity int32) error {
	return fe.cartSvc.AddItem(ctx, userID, productID, quantity)
}
...
func (fe *frontendServer) getShippingQuote(ctx context.Context, items []*model.CartItem, currency string) (*model.Money, error) {
	quote, err := fe.shippingSvc.GetQuote(ctx, items, currency)
	if err != nil {
		return nil, err
	}
	// shipping service might return USD, we might need to convert?
	// The new interface `GetQuote` has `currency` param, so assuming it returns in that currency.
	// But `handlers.go` uses `fe.convertCurrency(ctx, quote.GetCostUsd(), currency)`.
	// If my shipping service REST API returns CostUsd (as implied by my clients.go: CostUsd *model.Money),
	// then I need to convert.

	// My `clients.go` `GetQuote` returns `out.CostUsd`.
...
func (fe *frontendServer) getRecommendations(ctx context.Context, userID string, productIDs []string) ([]*model.Product, error) {
	resp, err := fe.recommendationSvc.ListRecommendations(ctx, userID, productIDs)
	if err != nil {
		return nil, err
	}
	// My `clients.go` ListRecommendations already returns `[]*model.Product` by fetching them.
	// So I don't need to fetch them here again.
	// Original logic fetched IDs then fetched products. My client does that.
